...
- **summarizer.py** — `summarize_articles()` returns a `SummarizeResult` dataclass (content + token usage). OpenAI SDK client, optionally configured with a custom `base_url` for OpenAI-compatible providers. Model name from `LLM_MODEL` env var (default: `gemini-2.5-pro`). Validates the SDK response is a `ChatCompletion` (the SDK uses lenient Pydantic construction and silently returns the raw value on a non-OpenAI body — e.g. wrong base URL or unknown model on OpenRouter — so we raise `SummarizationError` explicitly instead of crashing on `.choices`). Uses topic-specific line limits from `feeds.toml`, defaulting to (50, 100)
...
- **SSRF** (`utils.py`, `rss_fetcher.py`, `config.py`): Layered defense: (1) load-time `is_valid_url()` rejects bad schemes and literal non-global hosts using `is_non_routable_host()` with `not is_global` (covers private, loopback, link-local, CGN/RFC 6598, documentation, and benchmarking ranges), (2) fetch-time validation resolves DNS and rejects hostnames that currently resolve to non-global IPs, (3) manual redirect following resolves relative `Location` headers and validates each hop. HTTPS-only scheme enforcement for feed URLs. Also rejects trailing dots, zone IDs, bare integers (hex/octal/decimal), `file://` schemes. Validation functions centralized in `utils.py`, used by both `config.py` and `rss_fetcher.py`
...
- `LLM_BASE_URL` — Base URL for the provider. Must be the OpenAI-compatible base path (e.g. `https://openrouter.ai/api/v1`, no trailing `/chat/completions`). Optional for OpenAI direct (SDK default). HTTP allowed for localhost only; HTTPS required for remote
- `LLM_MODEL` — Model name (default: `gemini-2.5-pro`). Provider-specific. On OpenRouter, names are prefixed (e.g. `google/gemini-2.5-flash`, `openai/gpt-4o-mini`); on direct providers, use the bare name (`gpt-4o`, `llama3`)

None needed for `--skip-summarize` local runs. In CI, `LLM_API_KEY` and `LLM_BASE_URL` are GitHub secrets; `LLM_MODEL` is set in `daily-news.yml` (currently `google/gemini-2.5-flash` via OpenRouter). `PAT_TOKEN` is also a GitHub secret.
